...
function Notifications() {
  const [notificationsTabSelected, setNotificationsTabSelected] = React.useState(1);
  const [newNotifications, setNewNotifications] = React.useState(null);
  ...
  return (
    <section className={`${s.notifications} navbar-notifications`}>
      ...
      <footer className={[s.cardFooter, 'text-sm', 'card-footer'].join(' ')}>
        <Button
          color="link"
          className={classnames({ disabled: isLoad }, s.btnNotificationsReload, 'btn-xs', 'float-right', 'py-0')}
